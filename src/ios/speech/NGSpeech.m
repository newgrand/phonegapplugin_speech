...
-(void)recordStatus:(int)status withSpeexFile:(NSURL*) speexFileUrl{
    NSString* jsString = [NSString stringWithFormat:@"%@(%d,\"%@\");", @"cordova.require('com.newgrand.cordova.speech.Speech').onRecordStatus", status, [speexFileUrl absoluteString]];
    [self.commandDelegate evalJs:jsString];

}
//0 播放 1 播放完成 2出错
-(void)playStatus:(int)status withParm:(id) param{
    NSString* jsString = [NSString stringWithFormat:@"%@(%d,\"%@\");", @"cordova.require('com.newgrand.cordova.speech.Speech').onPlayStatus", status,param];
    [self.commandDelegate evalJs:jsString];
}
